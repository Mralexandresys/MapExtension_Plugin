...
DEFAULT_LOG_PATH="$SCRIPT_DIR/build_server.log"
DEFAULT_SDK_ROOT="$SCRIPT_DIR/StarRupture-Plugin-SDK"

usage() {
  cat <<'EOF'
Usage: ./build_server.sh <debug|release> [--sdk-root <path>] [--log] [--log-file <path>] [--summary] [--build-tag <tag>] [--build-author <author>]

Builds the MapExtension_Plugin C++ project (server configuration) using Windows MSBuild.
EOF
}
...
if [[ ! -d "$STARRUPTURE_SDK_BASE_DIR/Server/SDK" ]]; then
  echo "SDK serveur introuvable: $STARRUPTURE_SDK_BASE_DIR/Server/SDK" >&2
  exit 1
fi
